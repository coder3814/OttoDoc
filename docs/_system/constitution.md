...
Documentation is authored when a real task needs it — there is no autonomous documentation service. Required documentation lands in the same change or pull request as the implementation it describes, and the unit of assessment is that same change rather than each task inside it: a task that modifies the system notes its documentation impact in its own report and carries on, and before the change lands it receives one bounded documentation-impact assessment from the read-only coordinator covering the accumulated change as defined in [`process/coordinator.md`](process/coordinator.md). Assessing the unit the documentation must land in is what holds the two together, and it judges where the work actually ended up rather than where each step passed through. The coordinator reads the tasks' impact notes as evidence, never as a verdict. “No documentation change justified” is a valid and common result. The coordinator changes no files. When a durable reader need exists, it dispatches the documentation-only author and then a fresh-context reviewer that changes no files. It permits at most two revision-and-review cycles before asking the owner. Completion is reported briefly in natural language; material implementation concerns are separate from routine orchestration detail.
...
**Every supported platform gets an always-on surface.** The pre-landing assessment above is law for *every* change that modifies the system, so a platform that can only be reached by an agent choosing to reach for it cannot honor it. Which files carry each platform's surface is specified in [`lifecycle.md`](lifecycle.md). Where a platform offers a prompt-time extension point, two standing obligations are additionally injected into every prompt — route from the tree before forming a plan, evaluation, or recommendation, holding work and conversation to the glossary's canonical vocabulary (§2); and settle documentation for the accumulated change before that change lands — because static context alone has proven unreliable for judgment tasks; a platform without one carries only the static surface, a gap documented in `lifecycle.md` rather than papered over with more static text.
