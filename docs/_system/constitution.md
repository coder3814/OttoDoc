...
**Intake.** `docs/_intake/` is a durable, flat, inert holding area for non-authoritative source material awaiting an explicit processing request. Installation creates it, and upgrades restore it when it is missing, so it remains available even when empty. Two kinds of source live there side by side. A human may place any file type there that the active agent can read. An agent files a **change note** there — a Markdown file named `change-<YYYY-MM-DD>-<slug>.md`, written from `_system/templates/change-note.md`, recording what a system-modifying change cannot reveal about itself (§8). The lowercase `change-` prefix is what identifies the class to every actor, and a human who names a draft `change-*.md` will have it treated as a change note. Directories and unsupported files are rejected during assessment; filenames must be unique, and a collision is rejected rather than renamed or overwritten. Placement never starts processing. `OttoDoc intake [filename]` is the processing command. With one filename, the coordinator assesses that direct child of `_intake/`; with no filename, it assesses every file currently in `_intake/`. Paths, directories, multiple filenames, and filename patterns are invalid parameters.

After admitted documentation passes fresh-context review, the author deletes each consumed intake source in the same change as the documentation it produced. When processing concludes that a human draft should produce no live documentation, it reports that outcome and deletes the draft only after the owner's explicit approval — a human's work is never silently discarded. A change note is deleted on either outcome without approval: its commit is its archive, "no documentation change justified" is its common result, and when it yields no document its deletion is the whole documentation change. The `_intake/` directory itself remains in place after its contents are consumed. Generated indexes never enter intake because they contain no original knowledge.
...
Documentation is authored when a real task needs it — there is no autonomous documentation service — and authoring does not sit on the critical path of the change that creates the need. The unit of documentation is the change rather than each task inside it: the accumulated change as defined in [`process/coordinator.md`](process/coordinator.md), judged by where the work actually ended up rather than where each step passed through. A task that modifies the system notes its documentation impact and carries on. What the change owes before it lands — before it is committed or raised as a pull request — is a **change note** in `_intake/` (§6): a lead for the coordinator, not a draft document, recording what the code cannot reveal — purpose, the tasks' impact notes, decisions and the alternatives rejected, terms coined or resolved, facts the owner stated. The working agent writes it itself, opening it when the first system-modifying task in the change completes and keeping it current as later tasks finish, and it lands in the same commit as the code. Formatting-only, comment-only, generated-only, Git-only, and documentation-only changes need no note, and neither does a change confined to paths outside the system boundary (below). Documentation is then authored when the owner runs `OttoDoc intake`: the read-only coordinator assesses the noted changes against current repository state, reading the notes as evidence, never as a verdict. “No documentation change justified” is a valid and common result. When a durable reader need exists, the coordinator dispatches the documentation-only author and then a fresh-context reviewer that changes no files, and permits at most two revision-and-review cycles before asking the owner. `OttoDoc assess` remains the immediate path: the same assessment run now, over the branch diff and working tree, for an owner who wants a particular change's documentation authored before it lands. A change assessed that way owes no note — the assessment is what the note would have led to — and a note already begun for it is consumed like any other intake source. Completion is reported briefly in natural language; material implementation concerns are separate from routine orchestration detail.

**The system has a boundary.** The change-note obligation and the coordinator's assessment concern changes to the system the repository builds, not to the process that works on it. `docs/` and Git's own files (`.git/`, `.gitignore`, `.gitattributes`) are outside the system in every repository: they can never be a documentation subject, and holding `docs/` out is what keeps the loop from feeding itself — the documentation change that intake processing produces owes no note. Every other exclusion is the owner's judgment, listed in `.gitignore` syntax in `docs/.ottodocignore` ([`lifecycle.md`](lifecycle.md)): agent instruction files and platform adapters as installed, and whatever else in the repository belongs to the work process rather than to the thing being built. A change confined to paths outside the system owes no note and warrants no assessment. In a mixed change those paths are disregarded — the note does not record them and the coordinator does not weigh them.

Humans may supply rough documents or external material without first conforming to the engine. Agents normalize those inputs to this contract, preserve intended meaning, and ask before resolving material ambiguity. **Process lives with its actors under `_system/process/`**: workflow, coordination, authoring craft, and review criteria are canonical there. Each role also declares there the reasoning level its work demands, and adapters render that declaration in whatever form their platform accepts. Silently inheriting the driving session's model is not acceptable: what a role demands is a property of the role, not of whichever task happened to summon it. Platform-required files outside `_system/` are generated discovery adapters with no original process information.

**Every supported platform gets an always-on surface.** The change-note obligation above is law for *every* change that modifies the system, so a platform that can only be reached by an agent choosing to reach for it cannot honor it. Which files carry each platform's surface is specified in [`lifecycle.md`](lifecycle.md). Where a platform offers a prompt-time extension point, two standing obligations are additionally injected into every prompt — route from the tree before forming a plan, evaluation, or recommendation, holding work and conversation to the glossary's canonical vocabulary (§2); and keep the change note for the accumulated change current before that change lands — because static context alone has proven unreliable for judgment tasks; a platform without one carries only the static surface, a gap documented in `lifecycle.md` rather than papered over with more static text.
