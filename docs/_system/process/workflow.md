...
By default no role is dispatched before a system-modifying change lands. The working agent files a change note instead: when the first system-modifying task in the change completes, it creates `docs/_intake/change-<YYYY-MM-DD>-<slug>.md` from `docs/_system/templates/change-note.md` — dated at creation, slug drawn from the change's purpose — and appends to it as later tasks finish, so the tasks' documentation-impact notes accumulate in a file rather than in reports that context compaction can drop. The note is a lead for the coordinator, not a draft document: it records what the code cannot reveal, and the code itself is read at processing time. Before the change lands — before it is committed or raised as a pull request — the note is current, and it lands in the same commit as the code. The agent writes it at its own reasoning level; the coordinator, author, and reviewer are not summoned. Formatting-only, comment-only, generated-only, Git-only, and documentation-only changes need no note, so the documentation change that intake processing produces is itself exempt. Neither does a change confined to paths outside the system boundary (constitution §8) — `docs/`, Git's own files, and whatever `docs/.ottodocignore` lists — and a mixed change's note leaves those paths out.

Dispatch `doc-coordinator` for `OttoDoc assess`, for `OttoDoc intake`, and for every agent-driven documentation request. `assess` is the immediate path: the coordinator assesses the current change — the branch diff against its merge base together with the working tree — now, for an owner who wants a particular change's documentation authored before it lands; a change assessed this way owes no note, and a note already begun for it is consumed like any other intake source. `intake` is the deferred path: the coordinator assesses every change note and human draft in `docs/_intake/`, or the one named, on the owner's schedule. On either path the coordinator assesses impact itself and may conclude that no documentation change is justified. When work is needed, it dispatches `doc-author`, then a fresh-context `doc-reviewer`, re-dispatching the author with any findings for at most two revision cycles before asking the owner.
...
`docs/_intake/` is one flat folder holding two kinds of source side by side. Humans may place rough documents and external material there without conforming to the final structure. Agents file change notes there, recognized by the `change-` filename prefix. Filenames must be unique; do not add directories. Placement is inert. `OttoDoc intake <filename>` routes that one direct child through the coordinator; `OttoDoc intake` routes every file currently in intake. Reject paths, directories, multiple filenames, and filename patterns.

For a human draft, the author preserves intended meaning and human-provided external facts while normalizing kind, scope, summary-first structure, concision, links, and frontmatter; material ambiguity returns to the owner. For a change note, the coordinator assesses the change the note describes against current repository state, and several notes in intake are assessed as one batch. Consumed sources are deleted in the same docs change as the documentation they produced, or in a deletion-only docs change when none is produced. A human draft that yields no document is deleted only with the owner's approval; a change note is deleted on either outcome without it.
