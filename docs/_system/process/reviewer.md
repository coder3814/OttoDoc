...
7. **Intake sources.** A human draft's intended meaning is preserved without inheriting its structure or prose; a change note (`change-*.md`) is read as evidence, and its prose is never carried forward. Every consumed `_intake/` source is deleted in the docs change under review. A human draft producing no document requires recorded owner approval before deletion; a change note producing none is deleted without it.
