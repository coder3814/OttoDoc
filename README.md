...
Only changes to the system you are building trigger any of this. `docs/` and Git's own files are never part of that system, and `docs/.ottodocignore` lists — in `.gitignore` syntax — everything else that belongs to your work process rather than your product. Install seeds it with the agent instruction files and adapter directories (`CLAUDE.md`, `AGENTS.md`, `.claude/`, `.cursor/`, and the like) and OttoDoc's own CI workflow; add your own process files, or delete a line if one of those files genuinely is your product. A change confined to ignored paths files no note, the coordinator disregards them inside a larger change, and lint prints the active patterns on every run so an exclusion never goes unseen.
...
removes the engine, every platform, the record, the ignore file, and the CI check while preserving every document, index, asset, and `docs/_intake/`. The tree stays conformant, so reinstalling later restores the indexes byte for byte.
